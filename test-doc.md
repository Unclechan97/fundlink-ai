...
| 步骤 | 操作 | 说明 |
|------|------|------|
| 1 | 风控校验 | 调用内部风控系统查询客户信用评分、风控等级、反欺诈结果 |
| 2 | 获取客户信息 | 从核心系统获取客户姓名、证件、手机、职业等基本信息 |
| 3 | 获取账户信息 | 从支付系统获取收款银行卡号、联行号、开户行信息 |
| 4 | 渲染请求报文 | 将上游数据 + 内部系统数据填入 FreeMarker 模板，生成资金方要求的 JSON 报文 |
| 5 | 条件判断 | 风控等级为 A 直接放款，否则转人工审核 |
| 6 | 发送资金方 | 将渲染后的报文 POST 到银行接口 |

```
风控校验 → 获取客户信息 → 获取账户信息 → 渲染报文 → 条件判断(风控=A?)
                                                    ├─是→ 发送银行→结束(放款成功)
                                                    └─否→ 发送银行→结束(人工审核)
```

...
public class DeepSeekProvider implements LlmProvider {
    ...
    public DeepSeekProvider(
            @Value("${fundlink.llm.providers.deepseek.base-url:https://api.deepseek.com/v1}") String baseUrl,
        ...
        log.info("[LLM] DeepSeek provider ready  model={}", defaultModel);
    }
    ...
    public LlmResponse chat(LlmRequest request) {
        ...
        log.info("[LLM] >>> DeepSeek call  traceId={}  model={}  promptLen={}",
        ...
        try {
            ...
            if (statusCode != 200) {
                ...
                log.error("[LLM] <<< DeepSeek HTTP {}  traceId={}  latency={}ms  error={}",
                        statusCode, request.getTraceId(), elapsed, errorDetail);
                throw new RuntimeException(String.format(
                        "DeepSeek API returned HTTP %d: %s", statusCode, errorDetail));
            }
            ...
            if (choices == null || !choices.isArray() || choices.isEmpty()) {
                long elapsed = System.currentTimeMillis() - start;
                log.error("[LLM] <<< DeepSeek empty choices  traceId={}  latency={}ms  body={}",
                        request.getTraceId(), elapsed,
                        respBody.length() > 500 ? respBody.substring(0, 500) : respBody);
                throw new RuntimeException("DeepSeek returned empty choices — check API key and model availability");
            }
            ...
            log.info("[LLM] <<< DeepSeek done  traceId={}  latency={}ms  tokens={}/{}  contentLen={}",
                    request.getTraceId(), elapsed, in, out, content != null ? content.length() : 0);
            log.debug("[LLM] <<< DeepSeek content  traceId={}\n{}", request.getTraceId(), content);

            return LlmResponse.of(content, "deepseek", model,
            ...
            long elapsed = System.currentTimeMillis() - start;
            log.error("[LLM] <<< DeepSeek FAILED  traceId={}  latency={}ms  error={}",
